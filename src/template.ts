...
export function appHtml(panes: string): string {
  ...
  <div class="app">
    <div class="winbar">
      ...
      <button class="win-icon" id="winAbout" title="About Slayer T"><svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><line x1="12" y1="11" x2="12" y2="16"/><line x1="12" y1="7.7" x2="12" y2="7.8"/></svg></button>
      ...
      <aside class="sidebar">
        ...
        <div class="side-body">
          ...
          <div class="side-view side-prs" id="viewPRs">
            <div class="side-head"><span class="side-title">Pull Requests</span><button class="iss-repo-sel" id="prSideRepo" title="Pick the PR repository"></button><button class="files-up authf" id="prAuthor" style="display:none" title="Filter by author"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="8" r="3.5"/><path d="M5.5 20a6.5 6.5 0 0 1 13 0"/></svg></button><button class="files-up" id="prMap" title="Map PRs → review pipelines"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="7" height="5" rx="1"/><rect x="14" y="15" width="7" height="5" rx="1"/><path d="M10 6.5h4a3 3 0 0 1 3 3v5.5"/></svg></button><button class="files-up" id="prPull" title="Refresh pull requests"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 12a9 9 0 1 1-2.64-6.36"/><path d="M21 3v5h-5"/></svg></button></div>
            ...
            <input class="iss-search" id="prSearch" placeholder="filter by # or title…" spellcheck="false" style="display:none" />
            <div class="iss-state" id="prSort" style="display:none"><button class="iss-seg on" data-so="updated">Updated</button><button class="iss-seg" data-so="created">Created</button><button class="iss-seg" data-so="number">Number</button></div>
  ...
  <div class="modal" id="settings">
    ...
    <div class="modal-body">
      ...
      <div class="set-panel" data-panel="agents">
        ...
        <div class="field"><label>Repository dependencies <span class="mut">— auto-select deps by main repo</span></label><div class="row"><select id="rdMain"></select><button class="set" id="rdSave">Save mapping</button></div><input class="list-search" id="rdSearch" type="text" placeholder="Search dependency repos…" spellcheck="false" autocomplete="off" style="display:none"><div class="iss-deps rd-deps" id="rdDeps"></div><div class="rd-list" id="rdList"></div><div class="fhint">Pick a main repo, tick the repos it depends on, and Save. When you <b>Assign</b> an issue or <b>Validate</b> a task in that repo, those dependencies are pre-selected under <code>.deps/</code> (until you customize that specific issue/task).</div></div>
        <div class="field"><label>Brief notes <span class="mut">— extra prompts for Assign / Validate</span></label><div class="bn-list" id="bnList"></div><textarea class="pbrief-ta bn-input" id="bnInput" rows="2" spellcheck="false" placeholder="e.g. If the issue relates to other repositories or services, file a GitHub issue for each."></textarea><button class="set" id="bnAdd">+ Add note</button><div class="fhint">These appear as toggles in the Assign and Validate dialogs; checked notes are appended to the agent's brief. Tick <b>default</b> to pre-select a note.</div></div>
