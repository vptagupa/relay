...
| `src/renderer.ts` | **Large module (~2.0k lines) — being decomposed.** The UI shell + tab/pane engine + wiring: HTML template string, terminals/tabs, the split‑pane tree, blocks view, Library, Files, agent panel, bookmarks, history, command palette, settings, keyboard, drag/drop, themes, and the workspace switch/restore core. **Feature logic is moving out into modules** (see `blueprints.ts`) behind a dependency‑injection seam — add a feature to its module, not here. |
| `src/blueprints.ts` | Workspace **Templates** (save‑as / spawn‑from) + typed `[params]`. Owns the blueprint list + the Templates menu (`#tplMenu`) + its dialogs; the tab‑engine / workspace‑core functions it needs are **injected** via `initBlueprints(deps)`, so it never imports back into `renderer.ts`. The reference pattern for extracting the next feature. |
...
`renderer.ts` is still a large module over the global `state` singleton with manual DOM wiring, and there is **no automated test suite** (features are verified by driving the running app + checking `relay-error.log`). It is being **decomposed feature‑by‑feature**: `blueprints.ts` (Templates + typed params) is extracted behind a dependency‑injection seam; the **workspace switch/restore core** is the next candidate. **When you add or grow a feature, put it in a module (follow the `blueprints.ts` pattern) — do not add it to `renderer.ts`.** (The four panes are already generated from one `paneHtml()` + `selArr()` helper, no longer hand‑synced — that debt is paid.)
