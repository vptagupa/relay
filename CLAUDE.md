...
- **S — Single Responsibility.** One module/function/type, one reason to change. `pty.ts`, `blocks.ts`, `store.ts`, `blueprints.ts` are good examples; `renderer.ts` is the counter‑example — split cohesive concerns (tabs, layout, blocks view, library, agent panel, workspaces) out of it rather than growing it. **A feature module owns its own state + DOM events and receives what it needs from the renderer via an injected `deps` object** (see `blueprints.ts` → `initBlueprints`); it must never import back into `renderer.ts`. Adding a feature means creating/extending its module — never growing `renderer.ts`.
...
| `src/renderer.ts` | **Large module (~1.7k lines) — being decomposed.** The UI shell + tab/pane engine + wiring: HTML template string, terminals/tabs, the split‑pane tree, blocks view, Library, Files, agent panel, bookmarks, history, command palette, settings, keyboard, drag/drop, themes. **Feature logic lives in modules** (`blueprints.ts`, `workspaces.ts`) behind a dependency‑injection seam — add a feature to its module, not here. |
| `src/blueprints.ts` | Workspace **Templates** (save‑as / spawn‑from) + typed `[params]`. Owns the blueprint list + the Templates menu (`#tplMenu`) + its dialogs; the tab‑engine / workspace‑core functions it needs are **injected** via `initBlueprints(deps)`, so it never imports back into `renderer.ts`. |
| `src/workspaces.ts` | Named **workspaces**: the switch/restore core (`switchWorkspace`, `restoreWorkspaceSnapshot`, `teardownAllTabs`), bounded keep‑alive (detach on switch, LRU‑evict past `WARM_CAP`), the chip + switcher menu, management (duplicate / export / import / trust / rename / recolor / delete), per‑workspace root + theme, and `slayert://` deeplinks. Owns the workspace‑def list + active id; the ~17 tab‑engine functions it needs are **injected** via `initWorkspaces(deps)`, so it never imports back into `renderer.ts`. It *drives* `blueprints.ts` (it owns `switchWorkspace`, so it calls `initBlueprints`). The reference pattern for a DI‑seam feature module. |
...
`renderer.ts` is still a large module over the global `state` singleton with manual DOM wiring, and there is **no automated test suite** (features are verified by driving the running app + checking `relay-error.log`). It is being **decomposed feature‑by‑feature** behind a dependency‑injection seam: `blueprints.ts` (Templates + typed params) and `workspaces.ts` (the switch/restore core + management) are extracted; what remains in `renderer.ts` is the UI shell + the tab/pane engine those modules depend on. **When you add or grow a feature, put it in a module (follow the `workspaces.ts` / `blueprints.ts` pattern) — do not add it to `renderer.ts`.** (The four panes are already generated from one `paneHtml()` + `selArr()` helper, no longer hand‑synced — that debt is paid.)
